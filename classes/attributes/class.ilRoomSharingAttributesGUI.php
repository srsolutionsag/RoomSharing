...
class ilRoomSharingAttributesGUI {
	...
	protected function setSubTabs($a_active) {
		...
		if ($this->permission->checkPrivilege(PRIVC::ADMIN_ROOM_ATTRIBUTES)) {
			// Room attributes
			$ilTabs->addSubTab(ATTRC::ROOM_ATTRS, $this->lng->txt('rep_robj_xrs_attributes_for_rooms'), $this->ctrl->getLinkTargetByClass(ATTRC::ROOM_ATTRS_GUI, ATTRC::SHOW_ROOM_ATTR_ACTIONS));
		}
		if ($this->permission->checkPrivilege(PRIVC::ADMIN_BOOKING_ATTRIBUTES)) {
			// Booking attributes
			$ilTabs->addSubTab(ATTRC::BOOKING_ATTRS, $this->lng->txt('rep_robj_xrs_attributes_for_bookings'), $this->ctrl->getLinkTargetByClass(ATTRC::BOOKING_ATTRS_GUI, ATTRC::SHOW_BOOKING_ATTR_ACTIONS));
		}
