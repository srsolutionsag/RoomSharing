...
class ilRoomSharingDaVinciImport {
	...
	private function interpretR1Line($params) {
		array_push($this->rooms, array(
			'name' => $this->alterString($params[2]),
			'full_name' => $params[3],
			'cap' => $params[5],
			'type' => $params[8]
		));
	...
	private function interpretU2Line($params) {
		...
		for ($i = 0; $i < ($params[2]); $i ++) {
			...
			array_push($this->appointments, array(
				'id' => $params[1],
				'day' => $day,
				'start' => $startTime,
				'end' => $endTime,
				'room' => $this->alterString($roomShrt),
				'prof' => $this->alterString($profShrt),
				'subject' => $this->currentCourse,
				'classes' => $this->current_classes,
				'week' => $this->current_weekly_rotation
			));
		}
	}
	...
	private function addDaVinciBooking($day, $start, $end, $room, $prof, $subject, $classes, $usedWeek) {
		...
		if (($interval->format('%R')) === '-') {
			...
			if ($this->ilRoomSharingDatabase->getRoomWithName($room) !== array()) {
				$aBooking = $this->ilRoomSharingDatabase->getBookingIdForRoomInDateTimeRange($entry['from']['date'] . " " . $entry['from']['time'],
					$entry['to']['date'] . " " . $entry['to']['time'], $entry['room'], 0);
				...
				if ($aBooking === array() || $booking['bookingcomment'] !== $this->lng->txt("rep_robj_xrs_daVinci_import_tag")) {
					...
					try {
						$this->book->updateEditBooking($aBooking[0], $this->ilRoomSharingDatabase->getBooking($aBooking[0]), $this->ilRoomSharingDatabase->getAttributesForBooking($aBooking[0]), $this->ilRoomSharingDatabase->getParticipantsForBooking($aBooking[0]), $newBookingValues, $this->ilRoomSharingDatabase->getAttributesForBooking($aBooking[0]), $this->ilRoomSharingDatabase->getParticipantsForBooking($aBooking[0]), false);
					} catch (Exception $ex) {
					}
				}
			} else {
	...
	private function createSuccessMessage() {
		...
		if ($this->count_Bookings_created == 1) {
			$successText = $successText . ' ' . $this->count_Bookings_created . ' '
				. $this->lng->txt("rep_robj_xrs_daVinci_import_succ_booking_created");
		} elseif ($this->count_Bookings_created > 1) {
			$successText = $successText . ' ' . $this->count_Bookings_created . ' '
				. $this->lng->txt("rep_robj_xrs_daVinci_import_succ_bookings_created");
		}
