...
class ilRoomSharingDatabaseRoomAttribute {
	...
	public function getAttributesForRooms($a_room_ids) {
		$st = $this->ilDB->prepare('SELECT room_id, att.name, count FROM ' . dbc::ROOM_TO_ATTRIBUTE_TABLE . ' as rta LEFT JOIN '
			. dbc::ROOM_ATTRIBUTES_TABLE . ' as att ON att.id = rta.att_id WHERE ' . $this->ilDB->in("room_id", $a_room_ids) . ' AND pool_id = '
	...
	public function getMaxCountForAttribute($a_room_attribute) {
		...
		$attributIdSet = $this->ilDB->query('SELECT id FROM ' . dbc::ROOM_ATTRIBUTES_TABLE . ' WHERE name ='
			. $this->ilDB->quote($a_room_attribute, 'text') . ' AND pool_id = ' . $this->ilDB->quote($this->pool_id, 'integer'));
		...
		$valueSet = $this->ilDB->query('SELECT MAX(count) AS value FROM ' . dbc::ROOM_TO_ATTRIBUTE_TABLE . ' rta LEFT JOIN ' . dbc::ROOMS_TABLE
			. ' as room ON room.id = rta.room_id ' . ' WHERE att_id =' . $this->ilDB->quote($attributID, 'integer') . ' AND pool_id ='
	...
	public function getRoomAttribute($a_attribute_id) {
		$set = $this->ilDB->query('SELECT * FROM ' . dbc::ROOM_ATTRIBUTES_TABLE . ' WHERE id = ' . $this->ilDB->quote($a_attribute_id, 'integer')
	...
	public function getAttributesForRoom($a_room_id) {
		$set = $this->ilDB->query('SELECT id, att.name, count FROM ' . dbc::ROOM_TO_ATTRIBUTE_TABLE . ' rta LEFT JOIN ' . dbc::ROOM_ATTRIBUTES_TABLE
			. ' as att ON att.id = rta.att_id' . ' WHERE room_id = ' . $this->ilDB->quote($a_room_id, 'integer') . ' AND pool_id ='
			. $this->ilDB->quote($this->pool_id, 'integer') . ' ORDER BY att.name');
		$attributes = array();
		while ($row = $this->ilDB->fetchAssoc($set)) {
			$attributes[] = $row;
		}
	...
	public function deleteAllAttributesForRoom($a_room_id) {
		return $this->ilDB->manipulate('DELETE FROM ' . dbc::ROOM_TO_ATTRIBUTE_TABLE . ' WHERE room_id = '
	...
	public function insertAttributeForRoom($a_room_id, $a_attribute_id, $a_amount) {
		$this->ilDB->insert(ilRoomSharingDBConstants::ROOM_TO_ATTRIBUTE_TABLE, array(
	...
	public function getAllRoomAttributes() {
		$set = $this->ilDB->query('SELECT * FROM ' . dbc::ROOM_ATTRIBUTES_TABLE . ' WHERE pool_id = ' . $this->ilDB->quote($this->pool_id, 'integer')
			. ' ORDER BY name ASC');
	...
	public function deleteRoomAttribute($a_attribute_id) {
		return $this->ilDB->manipulate('DELETE FROM ' . dbc::ROOM_ATTRIBUTES_TABLE . ' WHERE id = ' . $this->ilDB->quote($a_attribute_id, 'integer')
			. ' AND pool_id =' . $this->ilDB->quote($this->pool_id, 'integer'));
	}
	...
	public function insertRoomAttribute($a_attribute_name) {
		...
		$this->ilDB->insert(dbc::ROOM_ATTRIBUTES_TABLE, array(
	...
	public function deleteAttributeRoomAssign($a_attribute_id) {
		return $this->ilDB->manipulate('DELETE FROM ' . dbc::ROOM_TO_ATTRIBUTE_TABLE . ' WHERE att_id = '
			. $this->ilDB->quote($a_attribute_id, 'integer'));
	}
