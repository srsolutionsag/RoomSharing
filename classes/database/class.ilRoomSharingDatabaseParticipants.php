...
class ilRoomSharingDatabaseParticipants {
	...
	public function insertBookingParticipant($a_insertedId, $a_participantId) {
		$this->ilDB->insert(dbc::BOOKING_TO_USER_TABLE, array(
	...
	public function updateBookingParticipant($a_insertedId, $a_participantId) {
		$this->ilDB->update(dbc::BOOKING_TO_USER_TABLE, array(
				'user_id' => array( 'integer', $a_participantId )
			), array(
				'booking_id' => array( 'integer', $a_insertedId ),
			));
	...
	public function getParticipantsForBooking($a_booking_id) {
		$set = $this->ilDB->query('SELECT users.firstname AS firstname,' . ' users.lastname AS lastname, users.login AS login,'
			. ' users.usr_id AS id FROM ' . dbc::BOOK_USER_TABLE . ' user ' . ' LEFT JOIN usr_data AS users ON users.usr_id = user.user_id'
			. ' WHERE booking_id = ' . $this->ilDB->quote($a_booking_id, 'integer') . ' ORDER BY users.lastname, users.firstname ASC');
		$participants = array();
	...
	public function getParticipantsForBookingShort($a_booking_id) {
		$set = $this->ilDB->query('SELECT  users.login AS login' . ' FROM ' . dbc::BOOK_USER_TABLE . ' participants '
			. ' INNER JOIN usr_data AS users ON users.usr_id = participants.user_id' . ' WHERE booking_id = '
			. $this->ilDB->quote($a_booking_id, 'integer') . ' ORDER BY users.lastname, users.firstname ASC');
		$participants = array();
		while ($row = $this->ilDB->fetchAssoc($set)) {
	...
	public function deleteParticipation($a_user_id, $a_booking_id) {
		return $this->ilDB->manipulate('DELETE FROM ' . dbc::BOOK_USER_TABLE . ' WHERE user_id = ' . $this->ilDB->quote($a_user_id, 'integer')
	...
	public function deleteParticipations($a_user_id, $a_booking_ids) {
		$st = $this->ilDB->prepareManip('DELETE FROM ' . dbc::BOOK_USER_TABLE . ' WHERE user_id = ' . $this->ilDB->quote($a_user_id, 'integer')
	...
	public function getParticipationsForUser($a_user_id) {
		$set = $this->ilDB->query('SELECT booking_id FROM ' . dbc::BOOK_USER_TABLE . ' WHERE user_id = ' . $this->ilDB->quote($a_user_id, 'integer'));
