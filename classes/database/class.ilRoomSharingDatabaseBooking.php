...
class ilRoomSharingDatabaseBooking {
	...
	public function insertBooking($a_booking_attr_values, $a_booking_values, $a_booking_participants) {
		global $ilUser;
		$this->ilDB->insert(dbc::BOOKINGS_TABLE, array(
		...
		$this->ilRoomSharingDatabase->insertBookingAppointment($insertedId, $a_booking_values, $a_booking_values ['from'][0], $a_booking_values ['to'][0]);
	...
	public function insertBookingRecurrence($a_booking_attr_values, $a_booking_values, $a_booking_participants) {
		...
		$query = "INSERT INTO " . dbc::BOOKINGS_TABLE
			. " (id, date_from, date_to, seq_id, room_id, pool_id, user_id, subject, public_booking, bookingcomment) VALUES ";
		// create SQL query
		$newBookIds = array();
		...
		for ($i = 0; $i < $count_booking_values_from; $i ++) {
			...
			$newBookIds[] = $book_id;
			$query .= "(" . $this->ilDB->quote($book_id, 'integer') . ", " . $this->ilDB->quote($a_booking_values['from'][$i], 'timestamp') . ", "
				. $this->ilDB->quote($a_booking_values['to'][$i], 'timestamp') . ", " . $this->ilDB->quote($next_seq_id, 'integer') . ", "
				. $this->ilDB->quote($a_booking_values['room'], 'integer') . ", " . $this->ilDB->quote($this->pool_id, 'integer') . ", "
				. $this->ilDB->quote($ilUser->getId(), 'integer') . ", " . $this->ilDB->quote($a_booking_values['subject'], 'text') . ", "
	...
	public function deleteBooking($a_booking_id) {
		$this->ilDB->manipulate('DELETE FROM ' . dbc::BOOKINGS_TABLE . ' WHERE id = ' . $this->ilDB->quote($a_booking_id, 'integer')
			. ' AND pool_id =' . $this->ilDB->quote($this->pool_id, 'integer'));
		$this->ilDB->manipulate('DELETE FROM ' . dbc::BOOK_USER_TABLE . ' WHERE booking_id = ' . $this->ilDB->quote($a_booking_id, 'integer'));
	}
	...
	public function deleteBookings($booking_ids) {
		$st = $this->ilDB->prepareManip('DELETE FROM ' . dbc::BOOKINGS_TABLE . ' WHERE ' . $this->ilDB->in("id", $booking_ids) . ' AND pool_id ='
			. $this->ilDB->quote($this->pool_id, 'integer'));
		$this->ilDB->execute($st, $booking_ids);
		$st2 = $this->ilDB->prepareManip('DELETE FROM ' . dbc::BOOK_USER_TABLE . ' WHERE ' . $this->ilDB->in("booking_id", $booking_ids));
	...
	public function getSequenceAndUserForBooking($a_booking_id) {
		$set = $this->ilDB->query('SELECT seq_id, user_id  FROM ' . dbc::BOOKINGS_TABLE . ' WHERE id = '
	...
	public function getFilteredBookings(array $filter) {
		$query =
			'SELECT b.id, b.user_id, b.subject, b.bookingcomment,' . ' r.id AS room_id, b.date_from, b.date_to, b.seq_id FROM ' . dbc::BOOKINGS_TABLE
			. ' b ' . ' JOIN ' . dbc::ROOMS_TABLE . ' r ON b.room_id = r.id ' . ' WHERE (date_from >= '
			. $this->ilDB->quote(date('Y-m-d H:i:s'), 'timestamp') . ' OR date_to >= ' . $this->ilDB->quote(date('Y-m-d H:i:s'), 'timestamp') . ')'
		...
		if ($filter['attributes']) {
			foreach ($filter['attributes'] as $attribute => $value) {
				$query .= ' AND EXISTS (SELECT * FROM ' . dbc::BOOKING_TO_ATTRIBUTE_TABLE . ' ba ' . ' LEFT JOIN ' . dbc::BOOKING_ATTRIBUTES_TABLE
					. ' a ON a.id = ba.attr_id ' . ' WHERE booking_id = b.id AND name = ' . $this->ilDB->quote($attribute, 'text')
					. ' AND value LIKE ' . $this->ilDB->quote('%' . $value . '%', 'text') . ' ) ';
			}
		}
	...
	public function getBooking($a_booking_id) {
		$set = $this->ilDB->query('SELECT *' . ' FROM ' . dbc::BOOKINGS_TABLE . ' WHERE id = ' . $this->ilDB->quote($a_booking_id, 'integer')
			. ' AND pool_id =' . $this->ilDB->quote($this->pool_id, 'integer') . ' AND (date_from >= "' . date('Y-m-d H:i:s') . '"'
	...
	public function getAllBookingsForRoom($a_room_id) {
		$set = $this->ilDB->query('SELECT * FROM ' . dbc::BOOKINGS_TABLE . ' WHERE room_id = ' . $this->ilDB->quote($a_room_id, 'integer')
			. ' AND pool_id =' . $this->ilDB->quote($this->pool_id, 'integer'));
		$bookings = array();
		while ($row = $this->ilDB->fetchAssoc($set)) {
			$bookings[] = $row;
	...
	public function getAllBookingsIds() {
		$set = $this->ilDB->query('SELECT id FROM ' . dbc::BOOKINGS_TABLE . ' WHERE pool_id = ' . $this->ilDB->quote($this->pool_id, 'integer'));
	...
	public function getBookingIdsForRoomInDateimeRanges($a_room_id, $a_datetimes_from, $a_datetimes_to) {
		$query = 'SELECT id FROM ' . dbc::BOOKINGS_TABLE . ' WHERE room_id = ' . $this->ilDB->quote($a_room_id, 'integer');
		...
		if ($count == count($a_datetimes_to)) {
			...
			for ($i = 0; $i < $count; $i ++) {
				...
				$query .= ' (' . $this->ilDB->quote($a_datetime_to, 'timestamp') . ' > date_from' . ' AND '
					. $this->ilDB->quote($a_datetime_from, 'timestamp') . ' < date_to) OR';
				if ($i == $count - 1) {
	...
	public function updateBooking($a_booking_id, $a_booking_attr_values, $a_old_booking_attr_values, $a_booking_values, $a_old_booking_values, $a_booking_participants, $a_old_booking_participants) {
		...
		$fields = array(
			...
			'date_from' => array(
				'timestamp',
				$a_booking_values ['from'] ['date'] . " " . $a_booking_values ['from'] ['time']
			),
			'date_to' => array(
				'timestamp',
				$a_booking_values ['to'] ['date'] . " " . $a_booking_values ['to'] ['time']
			),
			'room_id' => array( 'integer', $a_booking_values ['room'] ),
			'user_id' => array( 'integer', $ilUser->getId() ),
			'subject' => array( 'text', $a_booking_values ['subject'] ),
	...
	public function getBookingIdForRoomInDateTimeRange($a_date_from, $a_date_to, $a_room_id, $a_booking_id) {
		$query = 'SELECT DISTINCT id FROM ' . dbc::BOOKINGS_TABLE . ' WHERE pool_id =' . $this->ilDB->quote($this->pool_id, 'integer') . ' AND '
			. ' room_id = ' . $this->ilDB->quote($a_room_id, 'text') . ' AND ' . ' id != ' . $this->ilDB->quote($a_booking_id, 'integer') . ' AND '
			. ' (' . $this->ilDB->quote($a_date_from, 'timestamp') . ' BETWEEN date_from AND date_to OR '
			. $this->ilDB->quote($a_date_to, 'timestamp') . ' BETWEEN date_from AND date_to OR date_from BETWEEN '
			. $this->ilDB->quote($a_date_from, 'timestamp') . ' AND ' . $this->ilDB->quote($a_date_to, 'timestamp') . ' OR date_to BETWEEN '
			. $this->ilDB->quote($a_date_from, 'timestamp') . ' AND ' . $this->ilDB->quote($a_date_to, 'timestamp') . ')';
	...
	public function deleteAllBookingsAssignedToRoom($a_room_id) {
		return $this->ilDB->manipulate('DELETE FROM ' . dbc::BOOKINGS_TABLE . ' WHERE room_id = ' . $this->ilDB->quote($a_room_id, 'integer')
			. ' AND pool_id =' . $this->ilDB->quote($this->pool_id, 'integer'));
	}
	...
	public function getBookingsForRoomInTimeSpan($room_id, $start, $end, $type) {
		...
		if ($type != 4) {
			$query .= ' WHERE ((date_from <= ' . $this->ilDB->quote($end->get(IL_CAL_DATETIME, '', 'UTC'), 'timestamp') . ' AND date_to >= '
				. $this->ilDB->quote($start->get(IL_CAL_DATETIME, '', 'UTC'), 'timestamp') . ') OR (date_from <= '
		...
		$query .= ' AND room_id = ' . $this->ilDB->quote($room_id, 'integer') . ' AND pool_id = ' . $this->ilDB->quote($this->pool_id, 'integer')
			. ' ORDER BY date_from';
