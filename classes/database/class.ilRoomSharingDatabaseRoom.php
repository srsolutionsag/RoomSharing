...
class ilRoomSharingDatabaseRoom {
	...
	public function getMaxSeatCount() {
		$valueSet = $this->ilDB->query('SELECT MAX(max_alloc) AS value FROM ' . dbc::ROOMS_TABLE . ' WHERE pool_id = '
	...
	public function getRoomIdsWithMatchingAttribute($a_attribute, $a_count) {
		$queryString = 'SELECT room_id FROM ' . dbc::ROOM_TO_ATTRIBUTE_TABLE . ' ra ' . 'LEFT JOIN ' . dbc::ROOM_ATTRIBUTES_TABLE
			. ' attr ON ra.att_id = attr.id WHERE name = ' . $this->ilDB->quote($a_attribute, 'text') . ' AND count >= '
			. $this->ilDB->quote($a_count, 'integer') . ' AND pool_id = ' . $this->ilDB->quote($this->pool_id, 'integer');
	...
	public function getAllRooms() {
		$resRooms = $this->ilDB->query('SELECT * FROM ' . dbc::ROOMS_TABLE . ' WHERE pool_id = ' . $this->ilDB->quote($this->pool_id, 'integer'));
		$rooms = array();
	...
	public function getAllRoomNames() {
		$resRooms = $this->ilDB->query('SELECT name FROM ' . dbc::ROOMS_TABLE . ' WHERE pool_id = ' . $this->ilDB->quote($this->pool_id, 'integer'));
		$roomNames = array();
		$row = $this->ilDB->fetchAssoc($resRooms);
		while ($row) {
			$roomNames[] = $row['name'];
	...
	public function getMatchingRooms($a_roomsToCheck, $a_room_name, $a_room_seats) {
		...
		if ($a_room_name || $a_room_name === "0") {
			$where_part = $where_part . ' AND name LIKE ' . $this->ilDB->quote('%' . $a_room_name . '%', 'text') . ' ';
		}
		if ($a_room_seats || $a_room_seats === 0.0) {
			$where_part = $where_part . ' AND max_alloc >= ' . $this->ilDB->quote($a_room_seats, 'integer') . ' ';
		}

		$st = $this->ilDB->prepare('SELECT room.id, name, max_alloc FROM ' . dbc::ROOMS_TABLE . ' room WHERE '
			. $this->ilDB->in("room.id", array_keys($a_roomsToCheck)) . $where_part . ' ORDER BY name');
	...
	public function getRoomName($a_room_id) {
		$roomNameSet = $this->ilDB->query('SELECT name FROM ' . dbc::ROOMS_TABLE . ' WHERE id = ' . $this->ilDB->quote($a_room_id, 'integer')
	...
	public function getRoomsBookedInDateTimeRange($a_datetimes_from, $a_datetimes_to, $a_room_id = NULL, $a_priority = NULL) {
		...
		if ($a_priority) {
			...
			$join_part = ' JOIN ' . dbc::CLASS_USER_TABLE . ' u ON b.user_id = u.user_id JOIN ' . dbc::CLASSES_TABLE . ' c ON c.id = u.class_id';
		}

		$query = 'SELECT DISTINCT room_id FROM ' . dbc::BOOKINGS_TABLE . ' b ' . $join_part . ' WHERE b.pool_id = '
			. $this->ilDB->quote($this->pool_id, 'integer');
	...
	public function getAllRoomIdsWhereSeatsAvailable($a_min_seats) {
		$resRoomIds = $this->ilDB->query('SELECT id FROM ' . dbc::ROOMS_TABLE . ' WHERE pool_id = ' . $this->ilDB->quote($this->pool_id, 'integer')
			. ' AND max_alloc >= ' . $this->ilDB->quote($a_min_seats, 'integer'));
		$room_ids = array();
		while ($row = $this->ilDB->fetchAssoc($resRoomIds)) {
			$room_ids[] = $row['id'];
		}
	...
	public function getRoomsWithFloorplan($a_file_id) {
		$set = $this->ilDB->query('SELECT name FROM ' . dbc::ROOMS_TABLE . ' WHERE file_id = ' . $this->ilDB->quote($a_file_id, 'integer')
			. ' AND pool_id =' . $this->ilDB->quote($this->pool_id, 'integer'));
		$rooms = array();
		while ($row = $this->ilDB->fetchAssoc($set)) {
			$rooms[] = $row;
	...
	public function insertRoom($a_name, $a_type, $a_min_alloc, $a_max_alloc, $a_file_id, $a_building_id) {
		$this->ilDB->insert(dbc::ROOMS_TABLE, array(
	...
	public function deleteRoom($a_room_id) {
		return $this->ilDB->manipulate('DELETE FROM ' . dbc::ROOMS_TABLE . ' WHERE id = ' . $this->ilDB->quote($a_room_id, 'integer')
			. ' AND pool_id =' . $this->ilDB->quote($this->pool_id, 'integer'));
	}
