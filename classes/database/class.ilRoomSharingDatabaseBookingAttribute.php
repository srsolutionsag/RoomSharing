...
class ilRoomSharingDatabaseBookingAttribute {
	...
	public function getAttributesForBooking($a_booking_id) {
		$set = $this->ilDB->query('SELECT value, attr.name AS name' . ' FROM ' . dbc::BOOKING_TO_ATTRIBUTE_TABLE . ' bta ' . ' LEFT JOIN '
			. dbc::BOOKING_ATTRIBUTES_TABLE . ' attr ' . ' ON attr.id = bta.attr_id' . ' WHERE booking_id = '
	...
	public function getAllBookingAttributes() {
		$set = $this->ilDB->query('SELECT * FROM ' . dbc::BOOKING_ATTRIBUTES_TABLE . ' WHERE pool_id = '
			. $this->ilDB->quote($this->pool_id, 'integer') . ' ORDER BY name ASC');
	...
	public function getAllBookingAttributeNames() {
		$set = $this->ilDB->query('SELECT name FROM ' . dbc::BOOKING_ATTRIBUTES_TABLE . ' WHERE pool_id = '
	...
	public function updateDelBookingAttributeAssign($a_booking_id, $a_booking_attr_key) {
		$this->ilDB->query('DELETE FROM ' . dbc::BOOKING_TO_ATTRIBUTE_TABLE . ' WHERE booking_id = ' . $this->ilDB->quote($a_booking_id, 'integer')
			. ' AND attr_id = ' . $this->ilDB->quote($a_booking_attr_key, 'integer'));
	}
	...
	public function deleteAttributeBookingAssign($a_attribute_id) {
		return $this->ilDB->manipulate('DELETE FROM ' . dbc::BOOKING_TO_ATTRIBUTE_TABLE . ' WHERE attr_id = '
	...
	public function deleteBookingAttribute($a_attribute_id) {
		return $this->ilDB->manipulate('DELETE FROM ' . dbc::BOOKING_ATTRIBUTES_TABLE . ' WHERE id = '
			. $this->ilDB->quote($a_attribute_id, 'integer'));
	}
