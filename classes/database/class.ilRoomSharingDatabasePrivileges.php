...
class ilRoomSharingDatabasePrivileges {
	...
	public function getClasses() {
		$set = $this->ilDB->query('SELECT * FROM ' . dbc::CLASSES_TABLE . ' WHERE pool_id = ' . $this->ilDB->quote($this->pool_id, 'integer'));
		$classes = array();
	...
	public function getClassNames() {
		$set = $this->ilDB->query('SELECT name FROM ' . dbc::CLASSES_TABLE . ' WHERE pool_id = ' . $this->ilDB->quote($this->pool_id, 'integer'));
		$class_names = array();
		while ($row = $this->ilDB->fetchAssoc($set)) {
	...
	public function getClassById($a_class_id) {
		$set = $this->ilDB->query('SELECT * FROM ' . dbc::CLASSES_TABLE . ' WHERE id = ' . $this->ilDB->quote($a_class_id, 'integer')
	...
	public function getPrivilegesOfClass($a_class_id) {
		$set = $this->ilDB->query('SELECT * FROM ' . dbc::CLASS_PRIVILEGES_TABLE . ' WHERE class_id = ' . $this->ilDB->quote($a_class_id, 'integer'));
	...
	public function setLockedClasses($a_class_ids) {
		if (ilRoomSharingNumericUtils::isPositiveNumber(count($a_class_ids))) {
			$st = $this->ilDB->prepareManip('UPDATE ' . dbc::CLASSES_TABLE . ' SET locked = 1 WHERE ' . $this->ilDB->in('id', $a_class_ids));

			$this->ilDB->execute($st, array_keys($a_class_ids));

			$st2 = $this->ilDB->prepareManip('UPDATE ' . dbc::CLASSES_TABLE . ' SET locked = 0 WHERE ' . $this->ilDB->in('id NOT', $a_class_ids));

			$this->ilDB->execute($st2, array_keys($a_class_ids));
		} else {
			$this->ilDB->manipulate('UPDATE ' . dbc::CLASSES_TABLE . ' SET locked = 0');
		}
	}
	...
	public function getAssignedClassesForUser($a_user_id, $a_user_role_ids) {
		$class_ids = array();
		$st = $this->ilDB->prepare('SELECT id FROM ' . dbc::CLASSES_TABLE . ' LEFT JOIN ' . dbc::CLASS_USER_TABLE
			. ' ON id = class_id WHERE pool_id = ' . $this->ilDB->quote($this->pool_id, 'integer') . ' AND (user_id = '
			. $this->ilDB->quote($a_user_id, 'integer') . ' OR ' . $this->ilDB->in("role_id", $a_user_role_ids) . ')');
	...
	public function getLockedClasses() {
		$set = $this->ilDB->query('SELECT id FROM ' . dbc::CLASSES_TABLE . ' WHERE pool_id = ' . $this->ilDB->quote($this->pool_id, 'integer')
			. ' AND locked = 1');
	...
	public function getPriorityOfClass($a_class_id) {
		$set = $this->ilDB->query('SELECT priority FROM ' . dbc::CLASSES_TABLE . ' WHERE id = ' . $this->ilDB->quote($a_class_id, 'integer'));
		$row = $this->ilDB->fetchAssoc($set);
	...
	public function setPrivilegesForClass($a_class_id, $a_privileges, $a_no_privileges) {
		if (ilRoomSharingNumericUtils::isPositiveNumber(count($a_class_id))) {
			...
			$this->ilDB->manipulate('UPDATE ' . dbc::CLASS_PRIVILEGES_TABLE . ' SET ' . $positive_set . $negative_set . ' WHERE class_id = '
				. $this->ilDB->quote($a_class_id, 'integer'));
		}
	}
	...
	public function insertClass($a_name, $a_description, $a_role_id, $a_priority, $a_copy_class_id) {
		$this->ilDB->insert(dbc::CLASSES_TABLE, array(
	...
	public function assignUserToClass($a_class_id, $a_user_id) {
		if (!$this->isUserInClass($a_class_id, $a_user_id)) {
			$this->ilDB->insert(dbc::CLASS_USER_TABLE, array(
	...
	public function getUsersForClass($a_class_id) {
		$set = $this->ilDB->query('SELECT user_id FROM ' . dbc::CLASS_USER_TABLE . ' WHERE class_id = ' . $this->ilDB->quote($a_class_id, 'integer'));
	...
	public function deassignUserFromClass($a_class_id, $a_user_id) {
		$this->ilDB->manipulate("DELETE FROM " . dbc::CLASS_USER_TABLE . " WHERE class_id = " . $this->ilDB->quote($a_class_id, 'integer')
	...
	public function deleteClassPrivileges($a_class_id) {
		$this->ilDB->manipulate("DELETE FROM " . dbc::CLASS_PRIVILEGES_TABLE . " WHERE class_id = " . $this->ilDB->quote($a_class_id, 'integer'));
	}
	...
	public function deleteClass($a_class_id) {
		...
		$this->ilDB->manipulate("DELETE FROM " . dbc::CLASSES_TABLE . " WHERE id = " . $this->ilDB->quote($a_class_id, 'integer') . ' AND pool_id ='
	...
	public function isUserInClass($a_class_id, $a_user_id) {
		$set = $this->ilDB->query('SELECT * FROM ' . dbc::CLASS_USER_TABLE . ' WHERE class_id = ' . $this->ilDB->quote($a_class_id, 'integer')
			. ' AND user_id = ' . $this->ilDB->quote($a_user_id, 'integer'));
	...
	public function getUserPriority($a_user_id) {
		$set = $this->ilDB->query('SELECT MAX(priority) AS max_priority FROM ' . dbc::CLASSES_TABLE . ' JOIN ' . dbc::CLASS_USER_TABLE
