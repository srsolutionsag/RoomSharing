...
class ilRoomSharingRooms {
	...
	public function getList(array $a_filter = NULL) {
		...
		if (array_key_exists("date", $this->filter) && array_key_exists("time_from", $this->filter)
			&& array_key_exists("time_to", $this->filter)
		) {
	...
	private function getRoomsWithMatchingAttributes() {
		...
		foreach ($this->filter ['attributes'] as $attribute => $attribute_count) {
			$count = $count + 1;
			$roomsWithAttrib = $this->getMatchingRoomsForAttributeAsArray($attribute, $attribute_count, $roomsWithAttrib);
		}
	...
	private function getMatchingRoomsForAttributeAsArray($a_attribute, $a_count, $a_roomsWithAttrib) {
		$matching = $this->ilRoomsharingDatabase->getRoomIdsWithMatchingAttribute($a_attribute, $a_count);
	...
	private function removeRoomsNotInTimeRange() {
		if (array_key_exists('priority', $this->filter)) {
			$roomsBookedInTimeRange = $this->getRoomsBookedInDateTimeRange($this->filter['datetimes']['from'], $this->filter['datetimes']['to'], NULL, $this->filter['priority']);
		} else {
			$roomsBookedInTimeRange = $this->getRoomsBookedInDateTimeRange($this->filter['datetimes']['from'], $this->filter['datetimes']['to']);
		}
	...
	private function removeRoomsNotMatchingNameAndSeats() {
		if (array_key_exists("room_name", $this->filter) && array_key_exists("room_seats", $this->filter)) {
			$res_rooms = $this->ilRoomsharingDatabase->getMatchingRooms($this->roomsMatchingAttributeFilters, $this->filter ["room_name"], $this->filter ["room_seats"]);
		} elseif (array_key_exists("room_name", $this->filter)) {
			$res_rooms = $this->ilRoomsharingDatabase->getMatchingRooms($this->roomsMatchingAttributeFilters, $this->filter ["room_name"], NULL);
		} elseif (array_key_exists("room_seats", $this->filter)) {
			$res_rooms = $this->ilRoomsharingDatabase->getMatchingRooms($this->roomsMatchingAttributeFilters, NULL, $this->filter ["room_seats"]);
		} else {
			$res_rooms = $this->ilRoomsharingDatabase->getMatchingRooms($this->roomsMatchingAttributeFilters, NULL, NULL);
		}
