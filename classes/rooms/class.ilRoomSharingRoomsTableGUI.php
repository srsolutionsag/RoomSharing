...
class ilRoomSharingRoomsTableGUI extends ilTable2GUI {
	...
	public function getItems(array $filter) {
		...
		if (count($data) == 0 && ($new_name || $new_name === "0") && ($old_name || $old_name === "0")
			&& $old_name !== $new_name
		) {
			...
			$message = $this->lng->txt('rep_robj_xrs_no_match_for') . " $old_name " . $this->lng->txt('rep_robj_xrs_found') . ". "
	...
	public function fillRow($a_set) {
		...
		if ($this->parent_cmd === "showSearchResults") {
			...
			if ($this->permission->checkPrivilege(PRIVC::ACCESS_SEARCH)) {
				$this->tpl->setVariable('LINK_ACTION', $this->ctrl->getLinkTargetByClass('ilobjroomsharinggui', 'showSearch'));
			}
		}
		...
		if ($this->permission->checkPrivilege(PRIVC::ACCESS_ROOMS) && $this->parent_cmd === "showRooms") {
			...
			if ($this->permission->checkPrivilege(PRIVC::DELETE_ROOMS)) {
				$this->tpl->setVariable('LINK_ACTION', $this->ctrl->getLinkTargetByClass('ilobjroomsharinggui', 'confirmDeleteRoom'));
