...
class ilRoomSharingPrivilegesGUI {
	...
	private function createPrivilegeLockConfirmationDialog($a_class_ids_of_ticked_locks) {
		...
		foreach ($new_locked_class_ids as $class_id) {
			...
			$confirmation_dialog->addItem("new_locked_class_ids", "", $classById["name"]);
		}
		$confirmation_dialog->addHiddenItem("locked_classes", json_encode($a_class_ids_of_ticked_locks));

		$confirmation_dialog->setConfirm($this->lng->txt("rep_robj_xrs_privileges_confirm_class_lock"), "lockClassesAfterConfirmation");
	...
	private function getClassIdsForEndangeredPrivilegeOfAccessingAndEditingPrivileges($a_classes_with_ticked_privileges) {
		...
		foreach ($possibly_endangered_class_ids as $class_id) {
			$ticked_privileges_of_endangered_class_to_be_set = !empty($a_classes_with_ticked_privileges[$class_id]) ? $a_classes_with_ticked_privileges[$class_id] : array();
			...
			$privileges_to_be_unset = $this->determinePrivilegesToBeUnset($current_privileges_of_possibly_endangered_class, $privileges_of_endangered_class_to_be_set);
	...
	private function isEndangeredOfRevokingAccessingAndEditingPrivileges($a_privileges_to_be_unset) {
		return in_array(strtolower(PRIVC::ACCESS_PRIVILEGES), $a_privileges_to_be_unset)
		|| in_array(strtolower(PRIVC::EDIT_PRIVILEGES), $a_privileges_to_be_unset);
	}
