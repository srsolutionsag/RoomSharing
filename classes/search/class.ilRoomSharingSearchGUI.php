...
class ilRoomSharingSearchGUI {
	...
	private function getFormInput($search_form) {
		...
		foreach ($room_attributes as $room_attribute) {
			$attr_value = $search_form->getInputFromSession("attribute_" . $room_attribute . "_amount", false);
	...
	private function createDateFormItem() {
		...
		if (!empty($date_given['date'])) {
			...
			if ($hr_from >= 24) {   //increase the day if 23:00 or later
...
//		}
	...
	private function getCurrentTime() {
		...
		if ($hr_from >= 24) {   // increase the day if 23:00 or later and set time from 00:00 to 01:00
